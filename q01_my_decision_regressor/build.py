...
def my_decision_regressor(X_train,X_test,y_train,y_test,param_grid):
    ...
    gcv = GridSearchCV(dt,param_grid,cv=5)
    model1 = gcv.fit(X_train,y_train)
    y_pred = model1.predict(X_test)
    estimator = gcv.get_params()['estimator']
    return model1.score(X_test,y_test),(gcv.best_params_)
    
c=my_decision_regressor(X_train,X_test,y_train,y_test,param_grid) 
c
